// =============================================================================
// AL FILO — ComponentPicker v6
// Erkul-style dark table modal. Now shows prices + shop locations.
// Fetches from /api/catalog with include=stats,shops.
// =============================================================================
...
// Type map: resolvedCategory → ItemType for the catalog API
...
interface CatalogItem {
  ...
  weaponStats?: Record<string, any> | null;
  shieldStats?: Record<string, any> | null;
  powerStats?: Record<string, any> | null;
  coolingStats?: Record<string, any> | null;
  quantumStats?: Record<string, any> | null;
  miningStats?: Record<string, any> | null;
  missileStats?: Record<string, any> | null;
  thrusterStats?: Record<string, any> | null;
  shopInventory?: Array<{
    priceBuy: number | null;
    priceSell: number | null;
    shop: { name: string; location: { name: string; parentName: string | null } };
  }>;
}
...
export function ComponentPicker({ hardpoint, currentItemId, onSelect, onClear, onClose }: ComponentPickerProps) {
  ...
  // Extract unified stats from type-specific tables
  ...
  const getBestShop = useCallback((item: CatalogItem): string | null => {
    ...
    const s = sorted[0];
    return s.shop.name + " · " + s.shop.location.name;
  }, []);
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "stat": av = getStatVal(getItemStats(a), hardpoint.resolvedCategory); bv = getStatVal(getItemStats(b), hardpoint.resolvedCategory); break;
  ...
  // Convert catalog item → store EquippedItem
  const handleItemSelect = useCallback((item: CatalogItem) => {
    const stats = getItemStats(item);
    const equipped: EquippedItem = {
      id: item.id, reference: item.reference, name: item.name,
      localizedName: item.localizedName, className: item.className,
      type: item.type, size: item.size, grade: item.grade,
      manufacturer: item.manufacturer, componentStats: stats,
    };
    onSelect(equipped);
  }, [getItemStats, onSelect]);

  const statLabel = getStatLabel(hardpoint.resolvedCategory);
      ...
      <div className="fixed inset-4 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-[720px] sm:max-h-[75vh] bg-zinc-950 border border-zinc-800/70 rounded-sm flex flex-col z-50 shadow-2xl shadow-black/60">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-800/50 flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-zinc-300 truncate">{hardpoint.resolvedCategory} · S{hardpoint.maxSize} {hardpoint.isFixed ? "Fixed" : "Gimbal"}</div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0">
          {loading && results.length === 0 ? (
            <div className="flex items-center justify-center py-12 text-zinc-600 text-sm">
              <div className="w-4 h-4 border-2 border-zinc-700 border-t-cyan-500 rounded-full animate-spin mr-2" />Loading...
            </div>
          ) : sorted.length === 0 ? (
            ...
            const sv = getKeyStat(hardpoint.resolvedCategory, itemStats);
            const price = getBestPrice(item);
            const shop = getBestShop(item);
            const rowClass = isCurrent ? "w-full flex items-center gap-1 px-4 py-2 text-left bg-cyan-500/5 cursor-default border-b border-zinc-800/20" : "w-full flex items-center gap-1 px-4 py-2 text-left hover:bg-zinc-800/30 cursor-pointer border-b border-zinc-800/20 transition-colors";

            return (
              <button key={item.id} onClick={() => handleItemSelect(item)} disabled={isCurrent} className={rowClass}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <span className="text-[13px] text-zinc-200 truncate">{item.localizedName || item.name}</span>
                    {isCurrent && <span className="text-[8px] text-cyan-500 tracking-wider">EQUIPPED</span>}
                  </div>
...
function ColHead({ label, k, cur, dir, toggle, cls }: { label: string; k: SortKey; cur: SortKey; dir: SortDir; toggle: (k: SortKey) => void; cls: string }) {
  const active = cur === k;
  return (
    <button onClick={() => toggle(k)} className={cls + " cursor-pointer hover:text-zinc-400 transition-colors select-none " + (active ? "text-zinc-400" : "")}>
...
function getStatLabel(cat: string): string {
  switch (cat) {
    ...
    case "MISSILE_RACK": return "DMG";
    default: return "Pwr";
  }
}

function getStatVal(stats: Record<string, any> | null, cat: string): number {
  if (!stats) return 0;
  switch (cat) {
    case "WEAPON": case "TURRET": return stats.dps ?? 0;
...
function gradeClass(g: string): string {
  switch (g.toUpperCase()) {
    case "A": return "text-[10px] font-mono font-bold text-amber-400";
    case "B": return "text-[10px] font-mono font-bold text-cyan-400";
    case "C": return "text-[10px] font-mono font-bold text-zinc-400";
    default: return "text-[10px] font-mono font-bold text-zinc-600";
  }
}
