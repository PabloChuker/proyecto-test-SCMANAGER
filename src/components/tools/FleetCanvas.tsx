...
export default function FleetCanvas() {
  ...
  // Stable view updater — reads from ref so it never goes stale in closures
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ nodes, view }));
  ...
  const addShip = useCallback((reference: string, name: string, manufacturer: string) => {
    const canvas = canvasRef.current;
    const { panX, panY, zoom } = viewRef.current;
    ...
    setNodes((prev) => [
      ...prev,
      {
        id: uid(), reference, name, manufacturer,
        x: cx - NODE_W / 2 + jitter(),
        y: cy - NODE_H / 2 + jitter(),
      },
    ]);
  }, []);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (d.type === "pan") {
      ...
      setNodes((prev) =>
        prev.map((n) =>
          n.id === d.shipId
            ? { ...n, x: d.startNodeX + dx / z, y: d.startNodeY + dy / z }
            : n,
        ),
      );
    }
  ...
  return (
    <div className="flex h-full overflow-hidden bg-zinc-950">
      ...
      <aside
        className={`
          relative z-20 flex-col bg-zinc-900/95 backdrop-blur border-r border-zinc-800
                ...
                <div className="divide-y divide-zinc-800/40">
                  {hangarShipList.map((s) => (
                    <SidebarRow
                      key={s.id}
                      name={s.shipName}
                      sub={s.insuranceType.replace(/_/g, " ")}
                      onAdd={() => addShip(s.shipReference, s.shipName, "")}
                    />
                  ))}
                </div>
              )
      ...
      >
        ...
        {nodes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="text-center space-y-3">
              <div className="text-zinc-800 text-5xl font-thin select-none">◇</div>
        ...
        <div className="absolute bottom-4 right-4 flex flex-col gap-1.5 z-10">
          <CanvasBtn title="Zoom in" onClick={() => doZoom(1)}>
            <ZoomIn className="w-4 h-4" />
          </CanvasBtn>
          <CanvasBtn title="Zoom out" onClick={() => doZoom(-1)}>
            <ZoomOut className="w-4 h-4" />
          </CanvasBtn>
          <CanvasBtn title="Reset view" onClick={() => applyView({ panX: 0, panY: 0, zoom: 1 })}>
...
function EmptyHint({ text }: { text: string }) {
  return (
    <div className="p-6 text-center text-zinc-600 text-xs whitespace-pre-line leading-relaxed">
      {text}
    </div>
  );
}
...
function ShipNode({
  ...
  const glbUrls = shipGlbCandidates(node.reference);
  ...
  return (
    ...
    >
      <div
        className="bg-zinc-900/90 border border-zinc-700/60 rounded-xl overflow-hidden hover:border-amber-500/40 transition-colors shadow-2xl"
        style={{ height: NODE_H }}
      >
        {/* 3D viewer */}
        <div style={{ height: VIEWER_H }}>
          <ShipViewer3D
        ...
        <div className="px-2.5 py-2 border-t border-zinc-800/80">
          ...
          {node.manufacturer && (
            <p className="text-[9px] text-zinc-500 truncate leading-tight mt-0.5 uppercase tracking-wider">
              {node.manufacturer}
            </p>
          )}
        </div>
