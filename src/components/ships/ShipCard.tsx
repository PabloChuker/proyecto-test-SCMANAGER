// =============================================================================
// AL FILO — ShipCard v2 (schema v2 compatible)
// Changes: ship.maxSpeed → ship.scmSpeed, all classNames single-line
// =============================================================================
        ...
        <div className="p-5">
          ...
          <div className="flex items-start justify-between gap-3 mb-4">
            <div className="min-w-0 flex-1">
              <h3 className="font-medium tracking-wide text-zinc-100 truncate text-[15px] group-hover:text-cyan-50 transition-colors duration-200">{ship.localizedName || ship.name}</h3>
              <p className="text-[11px] tracking-[0.15em] uppercase text-zinc-500 mt-1 group-hover:text-zinc-400 transition-colors">{ship.manufacturer || "Unknown"}</p>
            </div>
            <span className={"text-lg opacity-40 group-hover:opacity-80 transition-opacity duration-300 " + roleColor}>{roleIndicator.icon}</span>
          </div>
          {/* Stats */}
          ...
          <div className="flex items-center justify-between pt-3 border-t border-zinc-800/50">
            <span className="text-xs text-zinc-500 tracking-wide">{ship.ship?.role || ship.ship?.focus || ship.ship?.career || "Multi-role"}</span>
            <span className="text-[10px] text-zinc-600 font-mono px-1.5 py-0.5 rounded-sm bg-zinc-800/40">v{ship.gameVersion}</span>
          </div>
        </div>
