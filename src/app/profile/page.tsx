...
function ConfigPanel({ onClose, discordName, discordAvatar }: { onClose: () => void; discordName: string | null; discordAvatar: string | null }) {
        ...
        <div>
          ...
          {linkedOpen && (
            <>
              {/* Discord */}
              <div className="flex items-center gap-3 py-3 border-b border-zinc-800/20">
                <div className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center flex-shrink-0 overflow-hidden">
                  {discordAvatar ? (
                    <img src={discordAvatar} alt="" className="w-full h-full object-cover rounded-lg" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-zinc-500 mt-0.5">{discordName ?? "—"} · {t("discordLinked")}</div>
                </div>
...
export default function ProfilePage() {
  ...
  // Discord linked account — pull from profile (synced on login) or fall back
  // to user_metadata directly. Preview fallback only when there's no auth at all.
                        ...
                        <div className="mt-4 pt-3 border-t border-zinc-700/40">
                          ...
                          <div className="flex flex-wrap gap-2">
                            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-indigo-500/40 bg-indigo-500/10">
                              ...
                              <Check size={10} className="text-lime-400 flex-shrink-0" />
                            </div>
                          </div>
                        </div>
                      )}
